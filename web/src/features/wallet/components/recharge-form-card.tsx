...
export function RechargeFormCard({
  ...
  const showUserAwareReferralPrompt = showReferralReward && userHasInviter
  const showGenericReferralInfo = showReferralReward && !userHasInviter

  const referralMinMoney = referralReward?.min_paid_money ?? 30
  const referralBelowThreshold =
    showUserAwareReferralPrompt &&
    topupAmount > 0 &&
    topupAmount < referralMinMoney
  ...
  let referralPromptMessage: ReactNode
  if (referralBelowThreshold) {
  ...
  } else if (referralMeetsThreshold) {
    referralPromptMessage = (
      <div>
        {t(
  ...
  let referralRewardAlert: ReactNode = null
  if (showUserAwareReferralPrompt || showGenericReferralInfo) {
    referralRewardAlert = (
        ...
        <AlertDescription className='space-y-1'>
          ...
          <div className='text-xs opacity-90'>
            {t(
              ...
              {
                days: referralReward?.inviter_settle_delay_days ?? 7,
              }
            )}
          </div>
        </AlertDescription>
