...
import SimpleRatioTable from './components/SimpleRatioTable';
...
const OPTION_KEYS = [
  ...
  'group_ratio_setting.subscription_group_ratio',
  'UserUsableGroups',
  'GroupGroupRatio',
...
export default function GroupRatioSettings(props) {
  ...
  useEffect(() => {
    ...
    const nextInputs = { ...DEFAULT_INPUTS, ...currentInputs };
    nextInputs.SubscriptionGroupRatio =
      nextInputs['group_ratio_setting.subscription_group_ratio'] ||
      nextInputs.SubscriptionGroupRatio;
  ...
  const handleSubscriptionGroupRatioChange = useCallback((value) => {
    setInputs((prev) => ({
      ...prev,
      SubscriptionGroupRatio: value,
      'group_ratio_setting.subscription_group_ratio': value,
    }));
  }, []);
  ...
  const renderVisualMode = () => (
    <Form key='form-visual' values={inputs} style={{ marginBottom: 15 }}>
      ...
      <Form.Section text={t('订阅分组倍率')}>
        ...
        >
          {t(
            '当请求扣除订阅额度时使用这里的倍率；未配置的分组会回退到普通分组倍率，钱包扣费仍使用普通分组倍率。',
          )}
        </Text>
        <SimpleRatioTable
          key={`sgr_${dv}`}
          value={inputs.SubscriptionGroupRatio}
          onChange={handleSubscriptionGroupRatioChange}
        />
      </Form.Section>
        ...
        <Row gutter={16}>
          <Col xs={24} sm={16}>
            <Form.TextArea
              ...
              placeholder={t('为一个 JSON 文本，键为分组名称，值为倍率')}
              extraText={t(
                '订阅分组倍率仅在请求扣除订阅额度时生效。例如：{"vip": 0.7}。如果同一请求改为钱包扣费，则仍使用普通分组倍率。',
              )}
              field={'SubscriptionGroupRatio'}
              autosize={{ minRows: 6, maxRows: 12 }}
              trigger='blur'
              stopValidateWithError
              rules={[
        ...
        <Tabs.TabPane tab={t('分组管理')} itemKey='groups'>
          <div style={{ paddingTop: 20 }}>
            ...
            <GuideSection title={t('JSON 格式参考')}>
              ...
              <CodeBlock>{`{"vip": 0.7}`}</CodeBlock>
              <Paragraph
                size='small'
                type='tertiary'
                style={{ marginTop: 6, lineHeight: 1.8 }}
              >
                {t(
                  '同一个分组可以同时配置普通分组倍率和订阅分组倍率。请求扣订阅额度时使用订阅分组倍率，扣钱包额度时使用普通分组倍率。',
                )}
              </Paragraph>
              <Paragraph size='small' style={{ marginBottom: 4, marginTop: 8 }}>
                <Text strong code>
