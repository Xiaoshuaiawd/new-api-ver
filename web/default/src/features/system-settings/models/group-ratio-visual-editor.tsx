...
export const GroupRatioVisualEditor = memo(function GroupRatioVisualEditor({
  ...
  const [subscriptionEditData, setSubscriptionEditData] =
    useState<SimpleGroup | null>(null)
  ...
  const subscriptionRatioList = useMemo(() => {
    const map = parseRatioMap(subscriptionGroupRatio)
  ...
  const handleSubscriptionSave = useCallback(
    (name: string, value: string) => {
      const map = parseRatioMap(subscriptionGroupRatio)
      if (subscriptionEditData && subscriptionEditData.name !== name) {
        delete map[subscriptionEditData.name]
      }
      map[name] = Number.parseFloat(value)
      onChange('SubscriptionGroupRatio', JSON.stringify(map, null, 2))
      setSubscriptionDialogOpen(false)
      setSubscriptionEditData(null)
    },
    [subscriptionGroupRatio, subscriptionEditData, onChange]
  )
  ...
  const handleSubscriptionDelete = useCallback(
    ...
    [subscriptionGroupRatio, onChange]
  )
    ...
    <div className='space-y-4'>
      ...
      <SimpleGroupDialog
        ...
        onOpenChange={(open) => {
          ...
          if (!open) setSubscriptionEditData(null)
        }}
        onSave={handleSubscriptionSave}
        editData={subscriptionEditData}
      />
    </div>
  )
})
...
function SimpleGroupDialog({
  ...
  return (
    <Dialog
      ...
      title={editData ? t('Edit subscription group ratio') : t('Add subscription group ratio')}
      description={t('Configure the subscription quota multiplier for this group.')}
      contentHeight='auto'
        ...
        <div className='space-y-2'>
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              if (next === '' || Number.isFinite(Number(next))) setValue(next)
            }}
            placeholder='1.0'
          />
        </div>
