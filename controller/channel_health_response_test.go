...
func TestWithChannelsRuntimeHealthAddsHealthyAndOpenSnapshots(t *testing.T) {
	...
	*setting = operation_setting.ChannelHealthSetting{
	...
	require.Equal(t, service.ChannelHealthStateV2Healthy, items[0].RuntimeHealth.StateV2)
	require.Equal(t, 100, items[0].RuntimeHealth.TrafficPercent)
	...
	require.Equal(t, service.ChannelHealthStateV2Unavailable, items[1].RuntimeHealth.StateV2)
	require.Equal(t, 0, items[1].RuntimeHealth.TrafficPercent)
