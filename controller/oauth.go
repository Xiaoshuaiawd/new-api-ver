...
func GenerateOAuthCode(c *gin.Context) {
	...
	if request.DeviceFingerprint == "" {
		request.DeviceFingerprint = strings.TrimSpace(c.Query("device_fingerprint"))
	}
	if oauth.GetProvider(request.Provider) == nil ||
		...
		len(request.Aff) > 32 ||
		len(request.DeviceFingerprint) > 128 ||
...
func HandleOAuth(c *gin.Context) {
	...
	user, err := findOrCreateOAuthUser(c, provider, oauthUser, payload.AffiliateCode, payload.DeviceFingerprint)
	if err != nil {
...
func findOrCreateOAuthUser(c *gin.Context, provider oauth.Provider, oauthUser *oauth.OAuthUser, affiliateCode string, deviceFingerprint string) (*model.User, error) {
	user := &model.User{}
	...
	if inviterId > 0 {
		if err := model.CreateReferralInviteIfNeeded(inviterId, user.Id, affiliateCode, model.ReferralInviteSourceOAuth, c.ClientIP(), c.Request.UserAgent(), deviceFingerprint); err != nil {
