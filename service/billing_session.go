...
func (s *BillingSession) preConsume(c *gin.Context, quota int) *types.NewAPIError {
	...
	if err := s.funding.PreConsume(effectiveQuota); err != nil {
		...
		if errors.Is(err, model.ErrNoActiveSubscription) {
			return newSubscriptionBillingError("当前无可用订阅")
		}
		if errors.Is(err, model.ErrSubscriptionQuotaInsufficient) {
			return newSubscriptionBillingError("订阅额度不足")
		}
...
func newSubscriptionBillingError(message string) *types.NewAPIError {
...
func (s *BillingSession) syncRelayInfo() {
	...
	if sub, ok := s.funding.(*SubscriptionFunding); ok {
		...
		info.SubscriptionPriceAmount = sub.PriceAmount
	} else {
		info.SubscriptionId = 0
		info.SubscriptionPreConsumed = 0
		info.SubscriptionPriceAmount = 0
	}
}
